...
with open('../CromAndExtensions/data/key_order.json', 'r') as data:
    key_order_hash = json.load(data)

# Allow configuration of overrides for the mapping of ontology to python/json
with open('../CromAndExtensions/data/overrides.json') as data:
    property_overrides = json.load(data)
...
with open('../CromAndExtensions/data/crm-profile.json') as data:
    profile_flags = json.load(data)

invdom = etree.parse('data/inverses.xml')
...
for fn in files:
    ...
    dom = etree.parse('data/%s' % fn)
    process_classes(dom)
    process_props(dom)


# outdata = '\n'.join(['\t'.join(x) for x in stuff])
fh = codecs.open('../CromAndExtensions/data/crm_vocab.tsv', 'w', 'utf-8')
for l in stuff:
    name = l[0]
    line = '\t'.join(l) + "\n"
    if name in classXHash:
